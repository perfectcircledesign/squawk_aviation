module Refinery
  module CustomerFiles
    ...
    self.dragonfly_url_format = '/system/customer_files/:job/:basename.:format'
    ...
    class << self
      def datastore_root_path
        config.datastore_root_path || (Rails.root.join('public', 'system', 'refinery', 'customer_files').to_s if Rails.root)
      end
